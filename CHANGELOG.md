...
- **Lambda foreign-key mismatch warnings now use structured logging**: When a navigation property's foreign key configuration doesn't align with the parent key definition, the warning is emitted through the service logger instead of stdout.
- **Lambda any/all composite joins honor column mappings**: Composite key join predicates for lambda any/all now use metadata-resolved column names on both the parent and related tables, honoring custom `gorm:"column:..."` tags. For composite keys without complete `foreignKey:` tags, a naming convention fallback (`<NavProp>_<ParentKey>`) is used as a last resort.
...
- **Navigation property $count results now honor $search**: Navigation property collection counts now apply `$search` consistently, using FTS when available and in-memory search with narrowed columns when not, aligning `$count` with navigation collection results.
