...
type Product struct {
	...
	Name        string        `json:"Name" gorm:"not null;index" odata:"required,maxlength=100,searchable"`
	Description *string       `json:"Description" odata:"nullable,maxlength=500"` // Nullable description field
	Price       float64       `json:"Price" gorm:"not null;index" odata:"required,precision=10,scale=2"`
	CategoryID  *uint         `json:"CategoryID" gorm:"index" odata:"nullable"` // Foreign key for Category navigation property
