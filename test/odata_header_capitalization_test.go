...
// TestODataHeaderCapitalizationViaGet verifies headers are accessible via Get() method as well
// This test complements the exact capitalization test by verifying that the headers
// are still accessible through Go's canonical header access methods.
func TestODataHeaderCapitalizationViaGet(t *testing.T) {
	service, db := setupHeaderCapTestService(t)
	...
	tests := []struct {
		...
		{
			name:           "GET - OData-Version via Get()",
			method:         http.MethodGet,
			path:           "/HeaderCapTestProducts",
			expectedStatus: http.StatusOK,
			checkVersion:   true,
		...
		{
			name:           "POST minimal - Both via Get()",
			method:         http.MethodPost,
			path:           "/HeaderCapTestProducts",
			body:           map[string]interface{}{"name": "New Product", "price": 49.99},
			headers:        map[string]string{"Prefer": "return=minimal"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.checkVersion {
				// Verify OData-Version is accessible via Get() (Go will canonicalize the lookup)
				version := w.Header().Get("OData-Version")
				if version != "4.0" {
					t.Errorf("OData-Version via Get() = %v, want 4.0", version)
				}
			}

			if tt.checkEntityId {
				// Verify OData-EntityId is accessible via Get() (Go will canonicalize the lookup)
				entityId := w.Header().Get("OData-EntityId")
				if entityId == "" {
					t.Error("OData-EntityId via Get() is empty")
				}
			}
		})
	}
