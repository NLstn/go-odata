...
func TestODataIDFieldIntegration(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:              "Minimal metadata - collection without $select should have @odata.id",
			url:               "/Products",
			acceptHeader:      "application/json;odata.metadata=minimal",
			metadataLevel:     "minimal",
			shouldHaveODataID: true,
			expectedODataID:   "http://localhost:8080/Products(1)",
			description:       "Per OData v4 spec, entities in collections MUST include @odata.id in minimal metadata",
		},
		{
			name:              "Minimal metadata - single entity without $select should HAVE @odata.id",
			url:               "/Products(1)",
			acceptHeader:      "application/json;odata.metadata=minimal",
			metadataLevel:     "minimal",
			shouldHaveODataID: true,
			expectedODataID:   "http://localhost:8080/Products(1)",
			description:       "Per OData v4 spec, individual entities MUST include @odata.id in minimal metadata",
		},
		{
			name:              "Minimal metadata - collection with $select (key omitted from request but auto-included) should have @odata.id",
		...
		{
			name:              "Minimal metadata - single entity with $select should HAVE @odata.id",
			url:               "/Products(1)?$select=name,price",
			acceptHeader:      "application/json;odata.metadata=minimal",
			metadataLevel:     "minimal",
			shouldHaveODataID: true,
			expectedODataID:   "http://localhost:8080/Products(1)",
			description:       "Per OData v4 spec, individual entities MUST include @odata.id in minimal metadata, even with $select",
		},
		{
			name:              "Minimal metadata - collection with $select (key included) should have @odata.id",
		...
		{
			name:              "Minimal metadata - single entity with $select (key included) should HAVE @odata.id",
