...
// TestOptionalDollarPrefix_V40_NoDollarAccepted verifies that response
// negotiation does not disable 4.01 request query-option syntax.
func TestOptionalDollarPrefix_V40_NoDollarAccepted(t *testing.T) {
	service, db := setupTestService(t)
	db.Create(&TestProduct{ID: 1, Name: "Laptop", Price: 999.99})
	db.Create(&TestProduct{ID: 2, Name: "Mouse", Price: 29.99})

	// filter=Price gt 100 without $ remains accepted with OData-MaxVersion: 4.0.
	req := httptest.NewRequest(http.MethodGet, "/TestProducts?filter="+url.QueryEscape("Price gt 100"), nil)
	...
	if len(value) != 1 {
		t.Errorf("Expected 1 product (unprefixed filter should work in v4.0), got %d", len(value))
	}
}
...
// TestOptionalDollarPrefix_V40_MixedCaseDollarAccepted verifies that mixed-case
// system query options are accepted even when the response is negotiated as 4.0.
func TestOptionalDollarPrefix_V40_MixedCaseDollarAccepted(t *testing.T) {
	service, db := setupTestService(t)
	db.Create(&TestProduct{ID: 1, Name: "Laptop", Price: 999.99})

	// $TOP=1 (mixed case) remains valid request syntax in v4.0 response mode.
	req := httptest.NewRequest(http.MethodGet, "/TestProducts?$TOP=1", nil)
	req.Header.Set("OData-MaxVersion", "4.0")
	w := httptest.NewRecorder()
	...
	if w.Code != http.StatusOK {
		t.Errorf("Expected 200 OK for mixed-case $TOP in v4.0 response mode, got %d: %s", w.Code, w.Body.String())
	}
}
